...
# ---------------- NEXT WORKOUT (RECOMMENDER) ----------------
with tab_recommend:
    st.subheader("Suggested Next Workout")
    st.caption(
        "A greedy priority-queue algorithm (Python `heapq`) that always assigns the next "
        "slot to whichever muscle group is furthest below an equal-share training target, "
        "runs in O(k log g) for a k-exercise session over g muscle groups."
    )

    session_size = st.slider("Exercises in this session", min_value=2, max_value=6, value=4)

    deficits = recommender.muscle_group_deficits()
    if not deficits:
        st.info("Add some exercises in the Exercises tab first.")
    else:
        deficit_df = pd.DataFrame(
            sorted(deficits.items(), key=lambda kv: kv[1], reverse=True),
            columns=["Muscle Group", "Deficit (pct pts below target)"],
        )
        fig = go.Figure(data=[go.Bar(
        ...
        picks = recommender.recommend_next_workout(session_size=session_size)
        if not picks:
            st.info("No deficits to correct right now - training looks balanced.")
        else:
