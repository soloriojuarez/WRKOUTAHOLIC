...
def get_exercise_last_trained() -> dict:
    """Map of exercise name -> most recent workout_date it was logged on
    (missing/None if never trained). Feeds the recommender's recency scoring."""
    query = """
        SELECT e.name, MAX(w.workout_date) AS last_trained
        FROM exercises e
...
def get_exercise_avg_session_volume() -> dict:
    ...
    to estimate how much one more session of an exercise would help close a
    muscle-group deficit; exercises with no history simply won't have a key."""
    query = """
        SELECT e.name, AVG(sv.session_volume) AS avg_volume
        FROM exercises e
        JOIN (
            SELECT s.exercise_id, s.workout_id, SUM(s.weight * s.reps) AS session_volume
            FROM sets s
            GROUP BY s.exercise_id, s.workout_id
        ) sv ON sv.exercise_id = e.id
        GROUP BY e.id
    """
    with get_connection() as conn:
        rows = conn.execute(query).fetchall()
    return {name: vol for name, vol in rows}
